...
/**
 * Master promo coupons — admin CRUD.
 * GET    → list with redemption stats + last 6 redemptions each
...
export async function GET() {
  return handle(async () => {
    ...
    const totals = await db.couponRedemption.aggregate({ _sum: { amount: true } })
    return jsonOk({ items, totalGiven: totals._sum.amount ?? 0 })
  })
}
...
export async function POST(req: NextRequest) {
  return handle(async () => {
    ...
    if (exists) return jsonError(`Code "${code}" already exists`)
    ...
    const item = await db.coupon.create({
      data: { platformId: null, code, value, maxUses, expiresAt, note: b.note ? String(b.note).slice(0, 200) : null },
    })

...
const TYPE_KEYS: Record<string, DictKey> = {
  DEPOSIT: 'ctx.typeDeposit',
  REFUND: 'ctx.typeRefund',
  ORDER: 'ctx.typeOrder',
  PLAN: 'ctx.typePlan',
  ADDON: 'ctx.typeAddon',
  ADJUSTMENT: 'ctx.typeAdjustment',
  PAYOUT: 'ctx.typePayout',
}
          ...
          <Button
            ...
            onClick={() =>
              downloadCsv(
                ...
                [t('common.date'), t('ctx.type'), t('cord.description'), t('ctx.method'), t('ctx.amountUsd'), t('common.status')],
                transactions.map((tx) => [
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:max-w-md">
        <Card className="p-4 sm:p-4">
          <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
            ...
            <p className="text-[11px] font-bold uppercase tracking-wide">{t('ctx.moneyIn')}</p>
          </div>
        ...
        <Card className="p-4 sm:p-4">
          <div className="flex items-center gap-2 text-rose-500">
            ...
            <p className="text-[11px] font-bold uppercase tracking-wide">{t('ctx.moneyOut')}</p>
          </div>
      ...
      <Card className="p-0 sm:p-0">
        {fundsLoading && !funds ? (
          ...
          <div className="p-4 sm:p-6">
            <EmptyState icon={ReceiptText} title={t('ctx.noneTitle')} message={t('ctx.noneDesc')} />
          </div>
        ) : (
          <TableWrap>
            <table className="w-full text-left text-[13px]">
              <thead>
                <tr className="border-b border-zinc-200 dark:border-zinc-800 text-[11px] font-bold uppercase tracking-wide text-zinc-400 dark:text-zinc-500">
                  ...
                  <th className="px-3 py-3">{t('ctx.type')}</th>
                  <th className="px-3 py-3">{t('cord.description')}</th>
                  <th className="hidden px-3 py-3 md:table-cell">{t('ctx.method')}</th>
...
function TxRow({ tx, m }: { tx: Tx; m: (n: number) => string }) {
  ...
  return (
    <tr className="border-b border-zinc-100 dark:border-zinc-800/70 transition hover:bg-zinc-50/70 dark:hover:bg-zinc-900/50">
      ...
      <td className="whitespace-nowrap px-3 py-3"><Pill tone={tone}>{t(TYPE_KEYS[tx.type] ?? 'ctx.typeOrder')}</Pill></td>
        ...
        <span className="flex items-center gap-1.5">
          ...
          {tx.status === 'PENDING' && (
            <span className="flex shrink-0 items-center gap-0.5 rounded-full bg-amber-50 dark:bg-amber-950/40 px-1.5 py-0.5 text-[10px] font-bold text-amber-700 dark:text-amber-400">
              <Clock3 className="h-2.5 w-2.5" /> {t('status.PENDING')}
            </span>
          )}
        </span>
      </td>

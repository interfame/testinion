...
export function CategoriesSection() {
  ...
  const save = async () => {
    ...
    const ok = await mutate(
      ...
      { success: editing === 'new' ? 'Category created' : 'Category updated' },
    )
  ...
  const doDelete = async () => {
    if (!deleting) return
    const ok = await mutate(() => apiDel('/api/admin/categories', { id: deleting.id }), { success: 'Category deleted' })
  ...
  return (
    <div className="space-y-4">
      <PanelPageHeader
        ...
        description="The platform-wide service catalog every storefront can resell."
        actions={
            ...
            <div key={c.id} className="group flex items-center gap-3 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4 shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition hover:shadow-md">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-[11.5px] text-zinc-400 dark:text-zinc-500">
                  {c._count.services} service{c._count.services === 1 ? '' : 's'} · sort {c.sortOrder}
                </p>
              </div>
              <div className="flex shrink-0 items-center gap-1">
                <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" onClick={() => openEdit(c)} aria-label={`Edit ${c.name}`}>
                  <Pencil className="h-3.5 w-3.5" />
                </Button>
                <Button variant="outline" size="icon" className="h-8 w-8 rounded-full text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40" onClick={() => setDeleting(c)} aria-label={`Delete ${c.name}`}>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Categories group services in every storefront catalog.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="flex items-center gap-3 rounded-xl border border-zinc-200 dark:border-zinc-800 p-3">
              ...
              <p className="text-[12px] text-zinc-500 dark:text-zinc-400">Live preview of the icon + color used across catalogs.</p>
            </div>
            <div>
              <FieldLabel>Name</FieldLabel>
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Instagram Followers" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex items-end gap-2 pb-1">
                ...
                <Label htmlFor="cat-active" className="text-[12.5px] font-semibold text-zinc-700 dark:text-zinc-200">Active</Label>
              </div>
            </div>
          </div>
          <DialogFooter>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Categories with services cannot be deleted — move or remove their services first.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

...
function useRemove(type: string, refresh: () => void) {
  ...
  const doDelete = async () => {
    if (!deleting) return
    const ok = await mutate(() => apiDel('/api/admin/content', { id: deleting.id, type }), { success: 'Deleted' })
    ...
    <AlertDialog open={!!deleting} onOpenChange={(o) => !o && setDeleting(null)}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>This content will be removed from the master platform.</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
export function NewsSection() {
  ...
  const togglePin = async (n: NewsItem) => {
    const ok = await mutate(() => api.patch('/api/admin/content', { type: 'news', id: n.id, pinned: !n.pinned }), { success: n.pinned ? 'Unpinned' : 'Pinned' })
    if (ok) refresh()
  }

  return (
    <div className="space-y-4">
      <PanelPageHeader title="News" description="Announcements shown across every storefront." actions={
        <Button onClick={() => open('new')} className="h-9 rounded-full px-4 text-[13px] font-bold text-[var(--on-brand)]" style={{ background: 'var(--brand)' }}><Plus className="mr-1 h-4 w-4" /> New announcement</Button>
      } />
      {loading && !data ? (
        <div className="space-y-2">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-14 rounded-xl" />)}</div>
      ) : (data?.items.length ?? 0) === 0 ? (
        <AdminCard><EmptyState title="No news yet" hint="Publish your first platform announcement." /></AdminCard>
      ) : (
        <TableShell>
          <table className="w-full text-left text-[13px]">
            <thead>
              <tr className="border-b border-zinc-100 dark:border-zinc-800/70 text-[11px] uppercase tracking-wide text-zinc-400 dark:text-zinc-500">
                ...
                <th className="px-3 py-3 font-bold">Preview</th>
                <th className="px-3 py-3 font-bold">Pinned</th>
            ...
            <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800/60">
              {data?.items.map((n) => (
                <tr key={n.id} className="transition hover:bg-zinc-50/60 dark:hover:bg-zinc-900/40">
                  ...
                  <td className="px-3 py-3"><Switch checked={n.pinned} onCheckedChange={() => togglePin(n)} aria-label="Toggle pinned" /></td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" onClick={() => open(n)} aria-label="Edit"><Pencil className="h-3.5 w-3.5" /></Button>
                      <Button variant="outline" size="icon" className="h-8 w-8 rounded-full text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40" onClick={() => setDeleting({ id: n.id, label: n.title })} aria-label="Delete"><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader><DialogTitle>{editing === 'new' ? 'New announcement' : 'Edit announcement'}</DialogTitle><DialogDescription>Visible on the news feed of all platforms.</DialogDescription></DialogHeader>
          <div className="space-y-3">
            <div><FieldLabel>Title</FieldLabel><Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. New payment method: USDT TRC20" /></div>
            <div><FieldLabel>Body</FieldLabel><Textarea rows={5} value={body} onChange={(e) => setBody(e.target.value)} /></div>
            <div className="flex items-center gap-2"><Switch id="news-pin" checked={pinned} onCheckedChange={setPinned} /><Label htmlFor="news-pin" className="text-[12.5px] font-semibold text-zinc-700 dark:text-zinc-200">Pinned to top</Label></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditing(null)}>Cancel</Button>
            <Button onClick={save} disabled={!title.trim()} style={{ background: 'var(--brand)' }}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      {confirmEl}
...
export function FaqsSection() {
  ...
  return (
    <div className="space-y-4">
      <PanelPageHeader title="FAQs" description="Knowledge-base answers for the master platform." actions={
        <Button onClick={() => open('new')} className="h-9 rounded-full px-4 text-[13px] font-bold text-[var(--on-brand)]" style={{ background: 'var(--brand)' }}><Plus className="mr-1 h-4 w-4" /> New FAQ</Button>
      } />
      {loading && !data ? (
        <div className="space-y-2">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-14 rounded-xl" />)}</div>
      ) : (data?.items.length ?? 0) === 0 ? (
        <AdminCard><EmptyState title="No FAQs yet" /></AdminCard>
      ) : (
        <TableShell>
          <table className="w-full text-left text-[13px]">
            <thead>
              <tr className="border-b border-zinc-100 dark:border-zinc-800/70 text-[11px] uppercase tracking-wide text-zinc-400 dark:text-zinc-500">
                ...
                <th className="px-3 py-3 text-right font-bold">Sort</th>
                <th className="px-4 py-3 text-right font-bold">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800/60">
              {data?.items.map((f) => (
                <tr key={f.id} className="transition hover:bg-zinc-50/60 dark:hover:bg-zinc-900/40">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" onClick={() => open(f)} aria-label="Edit"><Pencil className="h-3.5 w-3.5" /></Button>
                      <Button variant="outline" size="icon" className="h-8 w-8 rounded-full text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40" onClick={() => setDeleting({ id: f.id, label: f.question })} aria-label="Delete"><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-3">
            <div><FieldLabel>Question</FieldLabel><Input value={question} onChange={(e) => setQuestion(e.target.value)} /></div>
            <div><FieldLabel>Answer</FieldLabel><Textarea rows={4} value={answer} onChange={(e) => setAnswer(e.target.value)} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><FieldLabel>Category</FieldLabel><Input value={category} onChange={(e) => setCategory(e.target.value)} /></div>
              <div><FieldLabel>Sort order</FieldLabel><Input type="number" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} /></div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditing(null)}>Cancel</Button>
            <Button onClick={save} disabled={!question.trim()} style={{ background: 'var(--brand)' }}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      {confirmEl}
...
export function PostsSection() {
  ...
  return (
    <div className="space-y-4">
      <PanelPageHeader title="Blog posts" description="SEO content published on the public master blog." actions={
        <Button onClick={() => open('new')} className="h-9 rounded-full px-4 text-[13px] font-bold text-[var(--on-brand)]" style={{ background: 'var(--brand)' }}><Plus className="mr-1 h-4 w-4" /> New post</Button>
      } />
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border p-3.5" style={{ borderColor: 'color-mix(in srgb, var(--brand) 35%, transparent)', background: 'color-mix(in srgb, var(--brand) 7%, transparent)' }}>
        <div className="flex items-center gap-2.5">
          ...
          <p className="text-[12.5px] font-semibold text-zinc-700 dark:text-zinc-200">Published posts are live on the public GrowthRush blog (footer → Blog).</p>
        </div>
      ...
      {loading && !data ? (
        ...
        <AdminCard><EmptyState title="No posts yet" /></AdminCard>
      ) : (
        <TableShell>
          <table className="w-full text-left text-[13px]">
            <thead>
              <tr className="border-b border-zinc-100 dark:border-zinc-800/70 text-[11px] uppercase tracking-wide text-zinc-400 dark:text-zinc-500">
                ...
                <th className="px-3 py-3 font-bold">Published</th>
                <th className="px-4 py-3 text-right font-bold">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800/60">
              {data?.items.map((p) => (
                <tr key={p.id} className="transition hover:bg-zinc-50/60 dark:hover:bg-zinc-900/40">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" title="View on blog" aria-label="View on blog" onClick={viewMasterBlog}><Eye className="h-3.5 w-3.5" /></Button>
                      <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" onClick={() => open(p)} aria-label="Edit"><Pencil className="h-3.5 w-3.5" /></Button>
                      <Button variant="outline" size="icon" className="h-8 w-8 rounded-full text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40" onClick={() => setDeleting({ id: p.id, label: p.title })} aria-label="Delete"><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader><DialogTitle>{editing === 'new' ? 'New post' : 'Edit post'}</DialogTitle><DialogDescription>Format the body with the toolbar, tweak the raw HTML or hit Preview. The eye button in the list opens the public blog.</DialogDescription></DialogHeader>
          <div className="space-y-3">
            <div><FieldLabel>Title</FieldLabel><Input
              value={form.title}
              onChange={(e) => {
            ...
            /></div>
            <div>
              <FieldLabel hint="editable">Slug</FieldLabel>
            ...
            <div><FieldLabel hint="optional">Excerpt</FieldLabel><Input value={form.excerpt} onChange={(e) => setForm({ ...form, excerpt: e.target.value })} /></div>
            <div><FieldLabel>Body</FieldLabel><RichEditor value={form.body} onChange={(html) => setForm((f) => ({ ...f, body: html }))} placeholder="Write your post here…" minRows={10} /></div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditing(null)}>Cancel</Button>
            <Button onClick={save} disabled={!form.title.trim()} className="text-[var(--on-brand)]" style={{ background: 'var(--brand)' }}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      {confirmEl}
...
export function PagesSection() {
  ...
  return (
    <div className="space-y-4">
      <PanelPageHeader title="CMS pages" description="Terms of service, privacy policy and other static pages." actions={
        <Button onClick={() => open('new')} className="h-9 rounded-full px-4 text-[13px] font-bold text-[var(--on-brand)]" style={{ background: 'var(--brand)' }}><Plus className="mr-1 h-4 w-4" /> New page</Button>
      } />
      {loading && !data ? (
        <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-14 rounded-xl" />)}</div>
      ) : (data?.items.length ?? 0) === 0 ? (
        <AdminCard><EmptyState title="No pages yet" /></AdminCard>
      ) : (
        <TableShell>
          <table className="w-full text-left text-[13px]">
            <thead>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader><DialogTitle>{editing === 'new' ? 'New page' : 'Edit page'}</DialogTitle><DialogDescription>Slug is generated from the title.</DialogDescription></DialogHeader>
          <div className="space-y-3">
            <div><FieldLabel>Title</FieldLabel><Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></div>
            <div><FieldLabel>Body</FieldLabel><Textarea rows={7} value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} /></div>
            <div>
              <FieldLabel>Status</FieldLabel>
              <Select value={form.status} onValueChange={(v) => setForm({ ...form, status: v })}>
                <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditing(null)}>Cancel</Button>
            <Button onClick={save} disabled={!form.title.trim()} style={{ background: 'var(--brand)' }}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      {confirmEl}

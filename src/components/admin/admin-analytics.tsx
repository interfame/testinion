    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {loading && !stats ? (
          ...
          <>
            <StatCard label="Revenue · 30 days" value={formatMoney(stats?.revenue30d ?? 0, cur)} icon={TrendingUp} sub="Money-in transactions" />
            <StatCard label="Orders total" value={compactNumber(stats?.ordersTotal ?? 0)} icon={Crown} sub={`${stats?.ordersToday ?? 0} today`} accent="#f59e0b" />
            <StatCard label="Resellers" value={compactNumber(stats?.resellers ?? 0)} icon={Medal} sub={`${stats?.platformsActive ?? 0} active platforms`} accent="#10b981" />
          </>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        {/* Revenue by day */}
        <AdminCard title="Revenue by day" description="Last 30 days · money-in">
        ...
        {/* Orders by category */}
        <AdminCard title="Orders by category" description={`Last ${ordersInScope} orders`} >
          {ordersLoading ? (
            <Skeleton className="h-[280px] rounded-xl" />
          ) : byCategory.length === 0 ? (
            <p className="py-16 text-center text-[13px] text-zinc-400 dark:text-zinc-500">No orders to analyze yet</p>
          ) : (
            <div className="gr-chart h-[280px] w-full text-zinc-400 dark:text-zinc-500">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart layout="vertical" data={byCategory} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    content={
                      <ChartTip
                        title="Orders"
                        format={(v, e) => `${v} orders · ${formatMoney(Number(e.payload?.revenue ?? 0), cur)}`}
                      />
                    }
                  />
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        {/* Top services */}
        <AdminCard title="Top services" description="By number of orders">
            ...
            <table className="w-full text-left text-[13px]">
              <thead>
                <tr className="border-b border-zinc-100 dark:border-zinc-800/70 text-[11px] uppercase tracking-wide text-zinc-400 dark:text-zinc-500">
                  ...
                  <th className="px-3 py-2 text-right font-bold">Rate /1k</th>
              ...
              <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800/60">
                ...
                {!stats?.topServices.length && (
                  <tr><td colSpan={4} className="py-8 text-center text-[13px] text-zinc-400 dark:text-zinc-500">No service orders yet</td></tr>
                )}
              </tbody>
            </table>
          </TableShell>
        ...
        {/* Reseller leaderboard */}
        <AdminCard title="Reseller leaderboard" description="Platforms ranked by clients + orders">
          <ol className="space-y-2">
            {platformsLoading ? (
              Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-12 rounded-xl" />)
            ) : leaderboard.length === 0 ? (
              <li className="py-10 text-center text-[13px] text-zinc-400 dark:text-zinc-500">No platforms yet</li>
            ) : (
              leaderboard.map((p, i) => {
                ...
                return (
                  <li key={p.id} className="flex items-center gap-3 rounded-xl border border-zinc-100 dark:border-zinc-800/70 px-3 py-2.5 transition hover:bg-zinc-50/60 dark:hover:bg-zinc-900/40">
                    ...
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-[12px] font-extrabold text-white" style={{ background: `linear-gradient(135deg, ${t.accent}, ${t.accent2})` }}>
                      {p.name.slice(0, 1).toUpperCase()}
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[13px] font-bold text-zinc-800 dark:text-zinc-100">{p.name}</p>
                      <p className="truncate text-[11px] text-zinc-400 dark:text-zinc-500">{p.owner?.email} · {p.theme} theme</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[12px] font-bold tabular-nums text-zinc-800 dark:text-zinc-100">{p.clientsCount} clients</p>
                      <p className="text-[11px] tabular-nums text-zinc-400 dark:text-zinc-500">{p.ordersCount} orders</p>
                    </div>
                    <span className="hidden h-8 w-1.5 rounded-full sm:block" style={{ background: `linear-gradient(${t.accent}, ${t.accent2})` }} />
                  </li>
                )
              })
            )}

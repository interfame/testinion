    ...
    >
      ...
      <CommandList className="gr-scroll max-h-[340px]">
        ...
        {serviceSearch && services && services.length > 0 && (
          <CommandGroup heading={t('cpal.orderService')}>
        ...
        {nav.map((section, i) => (
          <CommandGroup
            key={i}
            heading={section.title ? t('cpal.goToWith').replace('{x}', section.title) : t('cpal.goTo')}
          >
      ...
      <button onClick={openPalette} className="sr-only" aria-label={t('cpal.openAria')} tabIndex={-1} />
      <span className="sr-only">{t('cpal.pressAria').replace('{k}', kbd)}</span>
...
export function PaletteTrigger({ className }: { className?: string }) {
  ...
  return (
    <button
      ...
      aria-label={t('cpal.searchAria')}
      title={t('cpal.searchTitle')}
      className={
...
export function PaletteTriggerMobile({ className }: { className?: string }) {
  ...
  return (
    <button
      onClick={() => window.dispatchEvent(new CustomEvent('gr:palette'))}
      aria-label={t('cpal.searchAria2')}
      title={t('cpal.searchAria2')}
      className={
